...
// Variable used to keep track of which current waveform
// 0: Sine
// 1: Sawtooth
// 2: Triangular
uint32_t currentWave = 0;

//#region SINUSOIDAL LUT
uint32_t sin_LUT[NS] = {
		512,
		537.122649255638,
		562.184775848735,
...
uint32_t TIM2_Ticks = NS * F_SIGNAL; //(TIM2CLK / F_SIGNAL) / NS; // How often to write new LUT value
...
void EXTI0_1_IRQHandler(void)
{
	...
	// Used global vars: curr_ms & prev_ms
	...
	if (curr_ms - prev_ms >= 500) {
			...
			// Start DMA in IT mode on TIM2->CH1; Source is LUT and Dest is TIM3->CCR3
			// and sets the source to the current desired LUT & updates LCD
			switch (currentWave) {
				case 0:
					currentWave = 1;
